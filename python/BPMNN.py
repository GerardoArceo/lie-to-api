...
carpeta_modelos = './models/bpm/' #Carpeta o subcarpeta donde están los modelos
...
# Carga json y crea modelo clasificación
...
# Recopilación de modelos de datos
mod_v = modelo_v.predict(entradas) #Predice usando la Red verdad
mod_v = sc.inverse_transform(mod_v).reshape(-1)

mod_m = modelo_m.predict(entradas) #Predice usando la Red mentira
mod_m = sc.inverse_transform(mod_m).reshape(-1)

datos = datos[60:datos.size]

if (datos.size<=5): #Evita errores
